  ...
  <div class="invoice-a4 position-relative">
    ...
    <div class="objet">
      <div>
        ...
        <span style="text-transform: uppercase;">@if($clientdevis->objet) {{ $clientdevis->objet }} @else PRESTATION DE SERVICE @endif</span>
      </div>
    </div>
    ...
    <table class="table table-sm w-100 ms-auto totals" style="width: 100%;">
      <tbody>
        <tr>
          ...
          <td class="gras-td">NBRE DE PASSAGES</td>
          <!-- <td style="width:10%;" class="gras-td">QTE</td> -->
          <td class="gras-td">PU</td>
          <td class="gras-td">TOTAL</td>
        </tr>
        @foreach ($clientdevis->clientdevisprestations as $clientdevisprestation)
          <tr>
            <td style="text-align: left;">{{ $clientdevisprestation->designation }}</td>
            <td>{{ $clientdevisprestation->unite }}</td>
            <td>{{ $clientdevisprestation->nbre_passage }}</td>
            <td>
              {{ getpricefr($clientdevisprestation->prix_unitaire) }}
            </td>
            <td style="text-align: right;">{{ getpricefr($clientdevisprestation->prix_total) }}</td>
          </tr>
        @endforeach
    ...
    <div class="mt-2 notes">
      ...
      @if($clientdevis->delai_paiement) {{ $clientdevis->delai_paiement }} @else Comptant @endif à compter de la date de réception de la facture.
    </div>
