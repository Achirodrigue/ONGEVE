      ...
      <div class="page-header">
        <div class="row align-items-center mb-3">
          <div class="col-sm mb-2 mb-sm-0">
            <h1 class="page-header-title">Factures partiellement payés du fournisseur <span class="badge bg-soft-dark text-dark ms-2">{{ $fournisseur->nom }}</span></h1>
          
          </div>

          <div class="col-auto">
            @if($fournisseur->fournisseurfacturecomptables->count() > 0)
              <a class="btn btn-primary" href="{{ route('fournisseur.facture.individuel.export', ['fournisseur' => $fournisseur->id, 'statut' => 2]) }}">
                <img class="avatar avatar-xss avatar-4x3 me-2" src="{{ asset("dashboard/assets/svg/brands/excel-icon.svg") }}" alt="Image Description">
                format excel
              </a>
            @endif
            <a class="btn btn-primary" href="{{ route('admin.fournisseur.index') }}">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal">
          ...
          <ul class="nav nav-tabs page-header-tabs" id="pageHeaderTab" role="tablist">
            <li class="nav-item">
              <a class="nav-link" href="{{ route('admin.fournisseur.facture.impaye', $fournisseur) }}" tabindex="-1" aria-disabled="true">
                Factures Impayées
                <span class="badge bg-soft-dark text-dark rounded-pill ms-1">{{ FFI($fournisseur) }}</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link active" href="#">
                Factures Partielle
                <span class="badge bg-soft-dark text-dark rounded-pill ms-1">{{ CAFPT($fournisseur) }} / {{ FFPT($fournisseur) }}</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="{{ route('admin.fournisseur.facture.paye', $fournisseur) }}" tabindex="-1" aria-disabled="true">
                Factures Finalisées
                <span class="badge bg-soft-dark text-dark rounded-pill ms-1">{{ CAFPT($fournisseur) }} / {{ FFPY($fournisseur) }}</span>
              </a>
            </li>
          </ul>
          <!-- End Nav -->
      ...
      <div class="card card-table">
          ...
          @else
              ...
              <div class="card-header card-header-content-md-between p-4">
                <h3 class="fw-bold mb-0 text-center">Désolé! Aucune facture partiellement payé pour ce fournisseur sur la plateforme</h3>
              </div>
              <!-- End Header -->
          @endif
      </div>
