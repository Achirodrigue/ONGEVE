            ...
            <div class="page-header">
                <div class="row align-items-center mb-3">
                    <div class="col-sm mb-2 mb-sm-0">
                        <h1 class="page-header-title">Nombre de véhicules sur la plateforme <span class="badge bg-soft-dark text-dark ms-2">{{ $pvehicules->count() }}</span></h1>
                    </div>
                ...
                <div class="js-nav-scroller hs-nav-scroller-horizontal">
                    ...
                    <ul class="nav nav-tabs page-header-tabs" id="pageHeaderTab" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('packauto.vehicule.maintenance') }}" tabindex="-1" aria-disabled="true">
                                Véhicules en maintenance
                                <span class="badge bg-soft-dark text-dark rounded-pill ms-1">{{ vehiculeMaintenances()->count() }}</span>
                            </a>
                        </li>
                    </ul>
                    <!-- End Nav -->
                ...
                @if($pvehicules->count() > 0)
                        ...
                        <div class="mb-2 mb-md-0 w-100">
                            <form>
                                ...
                                <div class="input-group input-group-merge input-group-flush">
                                    ...
                                    <input id="datatableSearch" type="search" class="form-control" placeholder="Rechercher un véhicule" aria-label="Rechercher un véhicule">
                                </div>
                                <!-- End Search -->
                            </form>
                        </div>
                    ...
                    <div class="table-responsive datatable-custom">
                        <table id="datatable" class="table table-bordered table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table" style="width: 100%" data-hs-datatables-options='{
                            ...
                            <tbody>
                                @php $n=1; @endphp
                                @foreach($pvehicules as $pvehicule)
                                    @php $a=$n; @endphp
                                    <tr>
                                        ...
                                        <td class="fw-bold">
                                            {{ $pvehicule->date_achat ? \Carbon\Carbon::parse($pvehicule->date_achat)->format('d/m/Y') : 'Null' }}
                                        </td>
                                        <td class="fw-bold">
                                            <span class="badge 
                                                @if($pvehicule->statut == 'accidenté') bg-success 
                                                @elseif($pvehicule->statut == 'vendu') bg-info 
                                                @elseif($pvehicule->statut == 'maintenance') bg-danger 
                                                @else bg-secondary @endif">
                                                {{ ucfirst($pvehicule->statut) }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="text-center fw-bold">
                                            <div class="btn-group" role="group">
                                                <a class="btn btn-white btn-sm" href="{{ route('packauto.pvehicule.show', $pvehicule) }}">
                                                    <i class="bi-eye me-1"></i> {{ $pvehicule->pvehiculedocs->count() }}
                                                </a>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-top mt-1" aria-labelledby="productsEditDrop{{ $pvehicule->id }}down">                
                                                        ...
                                                        <a class="dropdown-item" href="{{ route('packauto.qrcode.emprunt.vehicule', $pvehicule) }}">
                                                            <i class="bi-printer me-1 dropdown-item-icon"></i> QR Code d'emprunt
                                                        </a>    
                                                        <a class="dropdown-item" href="{{ route('packauto.qrcode.panne.vehicule', $pvehicule) }}">
                                                            <i class="bi-printer me-1 dropdown-item-icon"></i> QR Code de panne
                                                        </a>
                                                    </div>
                                                </div>
                                                <!-- End Button Group -->
