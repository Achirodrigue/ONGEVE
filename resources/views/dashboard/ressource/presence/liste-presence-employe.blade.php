    ...
    <div class="content container-fluid">
      ...
      <div class="page-header">
        <div class="row align-items-center mb-3">
          <div class="col-sm mb-2 mb-sm-0">
            <h1 class="page-header-title">Liste de présence des employés du <span class="text-danger">{{ date('d-m-Y', strtotime($date)) }}</span></h1>
          </div>
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal">
          ...
          <ul class="nav nav-tabs page-header-tabs" id="pageHeaderTab" role="tablist">
            <li class="nav-item">
              <a class="nav-link" href="{{ route('ressource.liste.presence') }}" tabindex="-1" aria-disabled="true">
                Dates de présence
              </a>
            </li>
            <li class="nav-item">
      ...
      <div class="card card-table mb-5">
          ...
          <div class="card-header card-header-content-md-between">
            <h4 class="card-header-title">Date : <span class="text-danger">{{ date('d-m-Y', strtotime($date)) }}</span></h4>
          </div>
          <!-- End Header -->
                ...
                <tbody>
                  @foreach($presences as $presence)
                    <tr>
                      ...
                      <td class="fw-bold">{{ $presence->employe->nom }} {{ $presence->employe->prenom }}</td>
                      <td class="fw-bold">{{ $presence->employe->contact }}</td>
                      <td class="fw-bold">@if($presence->arrive) {{ $presence->arrive }} @else Aucune @endif</td>
                      <td class="fw-bold">@if($presence->depart) {{ $presence->depart }} @else Aucune @endif</td>
                    </tr>
                  @endforeach
                </tbody>
              </table>
          ...
          @else
              ...
              <div class="card-header card-header-content-md-between p-4">
                <h5 class="fw-bold mb-0 text-center">Désolé! Aucune présence employé pour ce jour sur la plateforme</h5>
              </div>
              <!-- End Header -->
          @endif   
      </div>
