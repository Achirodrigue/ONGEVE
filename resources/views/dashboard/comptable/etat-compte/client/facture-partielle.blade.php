      ...
      <div class="page-header">
        <div class="row align-items-center mb-3">
          <div class="col-sm mb-2 mb-sm-0">
            <h1 class="page-header-title">Etat des comptes client partiellement payés <span class="badge bg-soft-dark text-dark ms-2">{{ $clientdevis->count() }}</span></h1>
          </div>
          <!-- End Col -->

          <div class="col-auto">
            @if($clientdevis->count() > 0)
              <a class="btn btn-primary" href="{{ route('comptable.client.facture.general.export', ['statut' => 2]) }}">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal">
          ...
          <ul class="nav nav-tabs page-header-tabs" id="pageHeaderTab" role="tablist">
            <li class="nav-item">
              <a class="nav-link" href="{{ route('comptable.etat.compte.client.impaye') }}" tabindex="-1" aria-disabled="true" href="#">
                Factures Impayées
            ...
            <li class="nav-item">
              <a class="nav-link" href="{{ route('comptable.etat.compte.client.paye') }}" tabindex="-1" aria-disabled="true">
                Factures Finalisées
      ...
      <div class="card card-table">
                ...
                <tbody>
                  ...
                  @foreach($clientdevis as $clientdevis)
                    ...
                    <tr>
                      ...
                      <td class="fw-bold">@if($clientdevis->daterecfacture) {{ $clientdevis->daterecfacture }} @else Vide @endif</td>
                      <td class="fw-bold">@if($clientdevis->delai_paiement) {{ $clientdevis->delai_paiement }} @else Vide @endif</td>
                      <td class="fw-bold">{{ getpricefr($clientdevis->total_payer) }}</td>
                      <td class="fw-bold">{{ getpricefr($clientdevis->versement) }}</td>
                      ...
                      <td>
                        <div class="btn-group" role="group">
                          <a class="btn btn-white btn-sm" href="#" data-bs-toggle="modal" data-bs-target="#addtransactioncommande{{ $clientdevis->id }}">
                            <i class="bi-printer me-1"></i>Add Transaction
                          </a>
                          <a class="btn btn-white btn-sm" href="#" data-bs-toggle="modal" data-bs-target="#editclientdevis{{ $clientdevis->id }}">
          ...
          @else
              ...
              <div class="card-header card-header-content-md-between p-4">
                <h3 class="fw-bold mb-0 text-center">Désolé! Aucune facture client partiellement payé sur la plateforme</h3>
              </div>
              <!-- End Header -->
          @endif
      </div>
