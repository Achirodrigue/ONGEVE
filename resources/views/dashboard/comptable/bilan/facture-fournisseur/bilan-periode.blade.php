      ...
      <div class="page-header">
        <div class="row align-items-center mb-3">
          <div class="col-sm mb-2 mb-sm-0">
            <h1 class="page-header-title">Bilans des factures fournisseur de la periode ci-dessous <span class="badge bg-soft-dark text-dark ms-2">{{ $fournisseurfacturecomptables->count() }}</span></h1>
            <h3 class="page-header-title"><span class="text-danger">{{ $periode }}</span></h3>
          </div>

          <div class="col-auto">
            <a class="btn btn-primary" 
              @if($type == 0) href="{{ route('comptable.bilan.semaine.facture.fournisseur') }}"
              @elseif($type == 1) href="{{ route('comptable.bilan.mois.facture.fournisseur') }}"
              @elseif($type == 2) href="{{ route('comptable.bilan.trimestre.facture.fournisseur') }}"
              @else href="{{ route('comptable.bilan.annee.facture.fournisseur') }}" @endif
            >
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal">
          ...
          <ul class="nav nav-tabs page-header-tabs" id="pageHeaderTab" role="tablist">
            <li class="nav-item">
              <a class="nav-link @if($type == 0) active" href="#" @else " href="{{ route('comptable.bilan.semaine.facture.fournisseur') }}" tabindex="-1" aria-disabled="true" @endif>
                Bilans mensuels
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link @if($type == 1) active" href="#" @else " href="{{ route('comptable.bilan.mois.facture.fournisseur') }}" tabindex="-1" aria-disabled="true" @endif>
                Bilans par mois
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link @if($type == 2) active" href="#" @else " href="{{ route('comptable.bilan.trimestre.facture.fournisseur') }}" tabindex="-1" aria-disabled="true" @endif>
                Bilans trimestriels
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link @if($type == 3) active" href="#" @else " href="{{ route('comptable.bilan.annee.facture.fournisseur') }}" tabindex="-1" aria-disabled="true" @endif>
                Bilans annuels
              </a>
            </li>
          </ul>
          ...
          @if($fournisseurfacturecomptables->count() > 0)
            ...
            <div class="table-responsive datatable-custom">
              <table id="datatable" class="table table-bordered table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table" style="width: 100%" data-hs-datatables-options='{
                ...
                <tbody>
                  @forelse($fournisseurfacturecomptables as $fournisseurfacturecomptable)
                    <tr>
