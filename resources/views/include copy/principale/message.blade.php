    ...
    @if(session('errorP'))
        <div class="alert alert-errorP alert-warning alert-dismissible fade show position-fixed top-50 start-50 translate-middle d-flex align-items-center flex-column" role="alert" style="font-size: 0.9em; z-index: 1050; height: 20%; width: 50%;">
    ...
    @if(session('Mfacture'))
        ...
        @endphp
        <div class="alert alert-Mfacture alert-warning alert-dismissible fade show position-fixed top-50 start-50 translate-middle d-flex align-items-center flex-column" role="alert" style="font-size: 0.9em; z-index: 1050; height: 20%; width: 50%;">
            <span>{{ session('Mfacture') }}</span>
            <button type="button" class="btn-close ms-2" data-bs-dismiss="alert" aria-label="Fermer"></button>
            <a href="{{ route('pdf.commande', $commande) }}" target="_blank">
