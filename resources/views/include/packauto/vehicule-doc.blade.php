  <!-- edit New Category Modal -->
    <div class="modal fade" id="editVehiculeDocument{{ $pvehiculedoc->id }}" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <!-- Header --> 
                <div class="modal-header">
                    <h4 class="modal-title" id="createAKIKeyModalLabel">
                      Modifier le document <span class="text-danger">{{ $pvehiculedoc->pvehiculedocname->nom }}</span> 
                      du véhicule <span class="text-danger">{{ $pvehiculedoc->pvehicule->marque }}</span>
                    <h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <!-- End Header -->

                <form method="post" action="{{ route('packauto.vehicule.document.update', $pvehiculedoc) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <!-- Body -->
                <div class="modal-body">
                    <div class="row mb-4">
                        ...
                        <div class="col-sm">
                            ...
                            <input type="date" name="date_expiration" value="{{ $pvehiculedoc->date_expiration }}" class="form-control" id="date_expiration" placeholder="Entrer une date d'expiration" aria-label="Entrer une date d'expiration">
  ...
  <div class="modal fade" id="deleteVehiculeDocument{{ $pvehiculedoc->id }}" tabindex="-1" aria-labelledby="createAKIKeyModalLabel" role="dialog" aria-hidden="true">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <p class="fw-bold mb-0">
              ...
              <span class="text-danger">{{ $pvehiculedoc->pvehiculedocname->nom }}</span> 
              du véhicule <span class="text-danger">{{ $pvehiculedoc->pvehicule->marque }}</span> ??
            </p>
        </div>
        <!-- End Body -->
        ...
        <!-- Footer -->
        <form action="{{ route('packauto.vehicule.document.destroy', $pvehiculedoc) }}" method="POST">
        @csrf
        @method('DELETE')
