    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title" id="createAKIKeyModalLabel">Ajout de <span class="text-warning">{{ $produit->nom }}</span> à <span class="text-warning">{{ $particulierdevis->particulier->nom }}</span></h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <!-- End Header -->

        <!-- Body -->
        <form method="post" action="{{ route('commercial.devis.particulier.store.deux', ['particulierdevis'=>$particulierdevis, 'produit'=>$produit]) }}" enctype="multipart/form-data">
        @csrf
