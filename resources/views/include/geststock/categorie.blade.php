    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title" id="createAKIKeyModalLabel">Modifier l'entrepôt <span class="text-warning">{{ $categorie->nom }}</span></h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <!-- End Header -->

        <form method="post" action="{{ route('geststock.categorie.update', $categorie) }}" enctype="multipart/form-data">
          @csrf
          @method('PATCH')
          <!-- Body -->
  ...
  <div class="modal fade" id="deleteCategorie{{ $categorie->id }}" tabindex="-1" aria-labelledby="createAKIKeyModalLabel" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <!-- Body -->
        <div class="modal-body">
            <p class="fw-bold mb-0">Voulez-vous vraiment supprimer l'entrepôt <span class="text-warning">{{ $categorie->nom }}</span> ??</p>
        </div>
        <!-- End Body -->

        <!-- Footer -->
        <form action="{{ route('geststock.categorie.destroy', $categorie) }}" method="POST">
        @csrf
        @method('DELETE')
            <div class="modal-footer">
                <div class="row align-items-sm-center flex-grow-1 mx-n2 justify-content-end">
