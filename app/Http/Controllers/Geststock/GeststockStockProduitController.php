...
class GeststockStockProduitController extends Controller
{
    ...
    //stock
        ...
        {
            $produits = Produit::whereHas('entrepotcateg', function ($q) use ($categorie) {
                $q->where('categorie_id', $categorie->id);
            })->get();

            // dd($produits->count());

            return view('dashboard.geststock.historique-stock.par-entrepot', compact('produits','categorie'));
        }
        public function StockFamilleProduitUn(Entrepotcateg $entrepotcateg)
        {
            $produits = $entrepotcateg->produits()->orderBy('updated_at', 'desc')->get();

            return view('dashboard.geststock.historique-stock.par-famille', compact('produits','entrepotcateg'));
        }
        public function StockProduitDeux()
        {
            $produitses = Produitse::orderBy('updated_at', 'desc')->get();

            return view('dashboard.geststock.historique-stock.stock2', compact('produitses'));
        }
